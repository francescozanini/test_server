# Again creating the net, and training

# SAME IMPORTS AS TUTO4 - Not totally needed
...
# From TUTO4 just take the stored training data
...
with torch.no_grad():
    for i in tqdm(range(len(test_x))):
        ...
        net_out = net(test_x[i].view(-1, 1, 50, 50)).squeeze()
        predicted_class = torch.argmax(net_out)
        if predicted_class == real_class:
            correct += 1
        total += 1
print('Accuracy: {}'.format(round(correct/total), 3)) #0.664 in cheat
